...
final RegExp _id = RegExp(r'@?(([A-Za-z][A-Za-z0-9_]*-)*([A-Za-z][A-Za-z0-9_]*))');
final RegExp _string1 = RegExp(r"'((\\(['\\/bfnrt]|(u[0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F])))|([^'\\]))*'");
final RegExp _string2 = RegExp(r'"((\\(["\\/bfnrt]|(u[0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F])))|([^"\\]))*"');

Scanner scan(String text, {sourceUrl, bool asDSX = false}) => _Scanner(text, sourceUrl)..scan(asDSX: asDSX);

abstract class Scanner {
  List<TwigDartError> get errors;
        ...
        while (!_scanner.isDone) {
          ...
          if (ch == $lt) {
            ...
            if (_scanner.matches('/')) {
              ...
              var shouldBreak = (openTags.isEmpty || openTags.first != 'script');
      ...
      if (true) {
        ...
        while (true) {
          ...
          if (token.type == TokenType.lt) {
            ...
            else if (brackets.isNotEmpty && brackets.first.type == TokenType.lt) {
          ...
          } else if (token.type == (asDSX ? TokenType.rCurly : TokenType.rDoubleCurly)) {
            state = _ScannerState.freeText;
            break;
          }
