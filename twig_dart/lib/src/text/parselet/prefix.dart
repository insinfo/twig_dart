part of twig.src.text.parselet;
...
class NotParselet implements PrefixParselet {
  ...
  Expression parse(Parser parser, Token? token) {
    ...
    if (expression == null) {
      parser.errors.add(TwigDartError(
          TwigDartErrorSeverity.error, 'Missing expression after "!" in negation expression.', token!.span));
    }
...
class NewParselet implements PrefixParselet {
  ...
  Expression? parse(Parser parser, Token? token) {
    ...
    if (call == null) {
      parser.errors.add(TwigDartError(
          TwigDartErrorSeverity.error, '"new" must precede a call expression. Nothing was found.', call!.span));
      return null;
    } else if (call is Call) {
      return NewExpression(token, call);
    } else {
      parser.errors.add(TwigDartError(TwigDartErrorSeverity.error,
          '"new" must precede a call expression, not a(n) ${call.runtimeType}.', call.span));
      return null;
    }
  }
}
...
class StringParselet implements PrefixParselet {
  ...
  @override
  Expression parse(Parser parser, Token? token) => StringLiteral(token, StringLiteral.parseValue(token!));
}
...
class ArrayParselet implements PrefixParselet {
  ...
  Expression? parse(Parser parser, Token? token) {
    ...
    if (!parser.next(TokenType.rBracket)) {
      ...
      lastSpan ??= token!.span;
      parser.errors
          .add(TwigDartError(TwigDartErrorSeverity.error, 'Missing "]" to terminate array literal.', lastSpan));
      return null;
    }
...
class MapParselet implements PrefixParselet {
  ...
  Expression? parse(Parser parser, Token? token) {
    ...
    if (!parser.next(TokenType.rCurly)) {
      ...
      parser.errors.add(TwigDartError(TwigDartErrorSeverity.error, 'Missing "}" in map literal.', lastSpan));
      return null;
    }
...
class ParenthesisParselet implements PrefixParselet {
  ...
  Expression? parse(Parser parser, Token? token) {
    ...
    if (expression == null) {
      parser.errors.add(TwigDartError(TwigDartErrorSeverity.error, 'Missing expression after "(".', token!.span));
      return null;
    }

    if (!parser.next(TokenType.rParen)) {
      parser.errors.add(TwigDartError(TwigDartErrorSeverity.error, 'Missing ")".', expression.span));
      return null;
    }
