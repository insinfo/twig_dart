...
class ConditionalParselet implements InfixParselet {
  ...
  Expression? parse(Parser parser, Expression? left, Token? token) {
    ...
    if (ifTrue == null) {
      parser.errors
          .add(TwigDartError(TwigDartErrorSeverity.error, 'Missing expression in conditional expression.', token!.span));
      return null;
    }

    if (!parser.next(TokenType.colon)) {
      parser.errors
          .add(TwigDartError(TwigDartErrorSeverity.error, 'Missing ":" in conditional expression.', ifTrue.span));
      return null;
    }
    ...
    if (ifFalse == null) {
      parser.errors
          .add(TwigDartError(TwigDartErrorSeverity.error, 'Missing expression in conditional expression.', colon!.span));
      return null;
    }
...
class CallParselet implements InfixParselet {
  ...
  Expression? parse(Parser parser, Expression? left, Token? token) {
    ...
    if (!parser.next(TokenType.rParen)) {
      ...
      lastSpan ??= token!.span;
      parser.errors.add(TwigDartError(TwigDartErrorSeverity.error, 'Missing ")" after argument list.', lastSpan));
      return null;
    }
...
class IndexerParselet implements InfixParselet {
  ...
  Expression? parse(Parser parser, Expression? left, Token? token) {
    ...
    if (indexer == null) {
      parser.errors.add(TwigDartError(TwigDartErrorSeverity.error, 'Missing expression after "[".', left!.span));
      return null;
    }

    if (!parser.next(TokenType.rBracket)) {
      parser.errors.add(TwigDartError(TwigDartErrorSeverity.error, 'Missing "]".', indexer.span));
      return null;
    }
...
class MemberParselet implements InfixParselet {
  ...
  Expression? parse(Parser parser, Expression? left, Token? token) {
    ...
    if (name == null) {
      parser.errors
          .add(TwigDartError(TwigDartErrorSeverity.error, 'Expected the name of a property following "."', token!.span));
      return null;
    }
