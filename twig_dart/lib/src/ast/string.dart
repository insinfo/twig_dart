    ...
    for (int i = 0; i < codeUnits.length; i++) {
      ...
      if (ch == $backslash) {
        if (i < codeUnits.length - 5 && codeUnits[i + 1] == $u) {
          var c1 = codeUnits[i += 2], c2 = codeUnits[++i], c3 = codeUnits[++i], c4 = codeUnits[++i];
        ...
        if (i < codeUnits.length - 1) {
          ...
          throw TwigDartError(TwigDartErrorSeverity.error, 'Unexpected "\\" in string literal.', string.span);
        }
      } else {
