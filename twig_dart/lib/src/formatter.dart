...
class TwigDartFormatter {
  ...
  int _formatElement(Element element, int lineLength) {
    ...
    var attrParts = element.attributes.isEmpty ? <String>[] : element.attributes.map(_formatAttribute);
    var attrLen = attrParts.isEmpty ? 0 : attrParts.map((s) => s.length).reduce((a, b) => a + b);
    _applySpacing();
    _buffer.write(header);

    // If the line will be less than maxLineLength characters, write all attrs.
    var ll = lineLength + (element is SelfClosingElement ? 2 : 1) + header.length + attrLen;
    if (ll <= maxLineLength) {
    ...
    for (var c in element.children) {
      ...
      lll = _formatChild(c, lineLength + lll, isFirst: i == 1 || last is Element, isLast: i == element.children.length);
