...
Parameter convertParameter(ParameterElement e) {
  return Parameter((b) {
    b
      ...
      ..named = e.isNamed
      ..defaultTo = e.defaultValueCode == null ? null : Code(e.defaultValueCode);
  });
}

UnsupportedError _unsupported() => UnsupportedError('Not support in R/O build file system.');
...
class BuildFileSystem extends FileSystem {
  ...
  @override
  Future<FileSystemEntityType> type(String path, {bool followLinks = true}) => throw _unsupported();

  @override
  FileSystemEntityType typeSync(String path, {bool followLinks = true}) => throw _unsupported();
}
...
class BuildSystemFile extends File {
  ...
  @override
  Future<FileSystemEntity> delete({bool recursive = false}) => throw _unsupported();
  ...
  @override
  Future<RandomAccessFile> open({FileMode mode = FileMode.read}) => throw _unsupported();

  @override
  Stream<List<int>> openRead([int start, int end]) => throw _unsupported();

  @override
  RandomAccessFile openSync({FileMode mode = FileMode.read}) => throw _unsupported();

  @override
  IOSink openWrite({FileMode mode = FileMode.write, Encoding encoding = utf8}) => throw _unsupported();

  @override
  Directory get parent => BuildSystemDirectory(fileSystem, reader, package, fileSystem.path.dirname(path));
  ...
  @override
  Future<List<String>> readAsLines({Encoding encoding = utf8}) => throw _unsupported();

  @override
  List<String> readAsLinesSync({Encoding encoding = utf8}) => throw _unsupported();
  ...
  @override
  Stream<FileSystemEvent> watch({int events = FileSystemEvent.all, bool recursive = false}) => throw _unsupported();
  ...
  @override
  void writeAsBytesSync(List<int> bytes, {FileMode mode = FileMode.write, bool flush = false}) => throw _unsupported();
...
class BuildSystemDirectory extends Directory {
  ...
  Directory childDirectory(String basename) {
    return BuildSystemDirectory(fileSystem, reader, package, fileSystem.path.join(path, basename));
  }

  @override
  File childFile(String basename) {
    return BuildSystemFile(fileSystem, reader, package, fileSystem.path.join(path, basename));
  }
  ...
  @override
  Stream<FileSystemEntity> list({bool recursive = false, bool followLinks = true}) => throw _unsupported();

  @override
  List<FileSystemEntity> listSync({bool recursive = false, bool followLinks = true}) => throw _unsupported();

  @override
  Directory get parent {
    return BuildSystemDirectory(fileSystem, reader, package, fileSystem.path.dirname(path));
  }
